...
def calculate_psi(expected, actual, buckets=10):
    ...
    breakpoints = scale_range(expected)
...
def detect_drift(**context):
    ...
    if not os.path.exists(TRAINING_DATASET_PATH):
        X.to_csv(TRAINING_DATASET_PATH)
        return "no_drift"

    reference = pd.read_csv(TRAINING_DATASET_PATH)

    psi_scores = {}
    for col in X.columns:
        psi_scores[col] = calculate_psi(reference[col], X[col])

    avg_psi = np.mean(list(psi_scores.values()))
    print("PSI scores:", psi_scores)
    print("AVG PSI:", avg_psi)
